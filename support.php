...
// 2) сколько записей в базе
    $resSupport = getStmtResult($link, "SELECT * FROM `support`");//сделали выборку из базы
...
// 7) массив от 1 до КОЛИЧЕСТВО СТРАНИЦ
    ...
    //pr($arСolStr);WHERE id>0
    $prevPage = '';
    if($pages > 1){//если текущая стр. больше 1   
        $prevPage  = $pages - 1;//то в переменную запишется текущее число -1
    }
    //получить  следующую  стр
    $nextPage = '';
    if($pages < $colSTR){////если текущая стр. меньше общего кол-ва стр
        $nextPage = $pages + 1;//то в переменную запишется текущее число + 1
    }
    $is_nav = ($colSTR >1) ? true : false; //если количество стр больше 1  то true тогда показывать иначе false

    $query ="SELECT s.`title`, s.`text` FROM `support`s  LIMIT ?, ? ";
    $param = [$startStr, $numb];
    $ress= getStmtResult($link, $query, $param );//получаем из базыWHERE id>0 LIMIT 0, 3
