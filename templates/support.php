  ...
  <!--  блок со счетчиком страниц -->    
      <div>
        <p class="pages">
          ...
          <a href="?page=<?=$numStr-1;?>"><<</a><?//переход на предыдущую страница -1?>
          <?if($numStr > 1):?><?//если номер стр. больше 1?>
          <a href="?page=<?=$numStr-1;?>"><?=$numStr-1?></a><?//тогда показывать предыдущую стр. ?>
          <?endif;?>
          <a href="?page=<?=$numStr;?>"><?=$numStr?></a><?// текущая страница ?>
          <?if($numStr !=$quantityStr):?><?//если номер страницы не равен кол-ву стр. ?>
          <a href="?page=<?=$numStr+1;?>"><?=$numStr+1?></a><?//тогда к  текущая стр. +1 ?>
          <?endif;?>

          <a href="?page=<?=$numStr+1;?>">>></a><?//переходна следующую стр. стр+1?>
